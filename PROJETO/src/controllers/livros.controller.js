import { createService, findAllService, countLivros, findByIdService, searchByTitleService, byUserService, updateService, deletaService, likeService, deleteLikeService } from '../services/livros.service.js';
...
const deleta = async (req, res) => {
    try {
        ...
        if (String(livros.user._id) !== req.userId) {
            return res.status(400).send({ message: "Você não pode deletar esse livro!" });
        }

        await deletaService(id);
...
const like = async (req, res) => {
    try {
        ...
        const like = await likeService(id, userId);
        ...
        res.send({ message: "Você deu like no livro!" });
...
export { create, findAll, findById, searchByTitle, byUser, update, deleta, like };